...
def build_agent(config: dict, provider_override: Optional[str] = None):
    ...
    tool_policy = ToolPolicy(config.get("tool_policy", {}), workspace_dir=workspace_dir)
...
def cmd_webui(config: dict):
    ...
    print(f"AgentSeed WebUI 启动中：http://{host}:{port}")
...
def main():
    parser = argparse.ArgumentParser(
        prog="agentseed",
        description="AgentSeed — 极简 Python Agent Runtime",
    )
