...
- 4 个示例 skill：`hello-world / sqlite-sample / excel-table / api-doc-extractor`
- `CLI` 单次执行与交互式 `REPL`
...
`api-doc-extractor` skill 可以从 API 文档中提取 GET 接口、拼接请求参数、读取接口返回数据，并整理为 Markdown、CSV 或 JSON 文件。

先只解析请求计划：

```powershell
python skills/api-doc-extractor/scripts/run.py --doc API-CN1.pdf --format markdown --dry-run
```

执行接口读取并导出整理文件：

```powershell
python skills/api-doc-extractor/scripts/run.py --doc API-CN1.pdf --format markdown
```

导出的文件默认写入：

- `workspace/api-doc-extractor/raw/`：原始接口响应
- `workspace/api-doc-extractor/exports/`：整理后的 Markdown、CSV 或 JSON
- `workspace/api-doc-extractor/plans/`：本次运行摘要
...
- [skills/api-doc-extractor/SKILL.md](./skills/api-doc-extractor/SKILL.md)
...
用于读取 API 文档、构造只读 GET 请求、抓取接口数据，并导出为 Markdown、CSV 或 JSON。适合测试：

- 从 PDF/Markdown/txt 文档中提取接口地址
- 按请求参数表拼接接口 URL
- 把 JSON 响应按字段表扁平化整理成文件

示例：

```powershell
python skills/api-doc-extractor/scripts/run.py --doc API-CN1.pdf --format csv
```
...
- API 数据整理产物：`workspace/api-doc-extractor/`
