...
class Agent:
    ...
    def run(
        ...
        user_text = self._normalize_db_intent_text(user_text)
...
Database routing rule (project-specific):
- In this project, requests mentioning database intent (e.g., "数据库", "db", "mysql", "table", "表", "schema", "sql", "orders表") are considered related to pledgebox data by default.
- A bare table-name style input (for example: `orders`, `order_items`, `item_variants`) is also database intent by default.
- If skill `pledgebox-sync` exists in the catalog, you MUST call activate_skill(name="pledgebox-sync") before any database command.
- Only if `pledgebox-sync` is not available, fall back to direct built-in tools (such as bash/read).
...
{extra_rules_block}"""
    ...
    def _normalize_db_intent_text(self, user_text: str) -> str:
        text = (user_text or "").strip()
        if "pledgebox-sync" not in self.skill_loader.catalog:
            return user_text
        if _TABLE_NAME_RE.match(text):
            return (
                "Database table query intent.\n"
                "Use pledgebox-sync skill workflow to inspect this table.\n"
                f"User input: {text}"
            )
        return user_text
