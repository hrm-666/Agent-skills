"""
工具注册表 + 安全策略：统一管理所有内置工具的 schema、执行和安全管控
"""
import logging
import shlex
...
DEFAULT_POLICY_CONFIG: Dict[str, Any] = {
    ...
    "bash": {
        "blocked_patterns": [
            "rm -rf /", "rm -rf /*", "sudo", "shutdown",
            "reboot", "> /dev/", "mkfs", "dd if=",
        ],
        "confirm_patterns": ["rm ", "mv ", "chmod ", "chown "],
    },
    "write": {
...
class ToolPolicy:
    ...
    def check(self, tool_name: str, args: Dict[str, Any]) -> PolicyDecision:
        ...
        if self.config.get("default") == "block":
    ...
    def _check_write(self, path: str) -> PolicyDecision:
        ...
        if matched:
            return PolicyDecision.confirm(
                reason=f"写入该路径需要确认：{matched}", matched=matched,
            )
        return PolicyDecision.allow()
...
class ToolRegistry:
    ...
    def execute(self, name: str, arguments: dict, confirmed: bool = False) -> ToolResult:
        ...
        if decision.action == "block":
            ...
            return ToolResult.failure(
                "PolicyBlocked", decision.reason, recoverable=False,
        ...
        if decision.action == "confirm" and not confirmed:
            ...
            return ToolResult.failure(
                "ConfirmationRequired", decision.reason, recoverable=True,
                meta={
